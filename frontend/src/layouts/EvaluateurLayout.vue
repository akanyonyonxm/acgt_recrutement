...
const NAV = [
  { to: { name: 'eval-dossiers' }, icon: 'mdi-clipboard-text-search-outline', t1: 'Mes', t2: 'dossiers' },
]
  ...
  <v-navigation-drawer v-model="drawer" class="acgt-drawer" width="170"
    ...
    <div class="d-flex flex-column h-100">
      ...
      <v-list class="pa-0 flex-grow-1" nav density="comfortable">
        <v-list-item v-for="n in NAV" :key="n.t1" :to="n.to"
                     active-class="nav-actif" class="nav-carre" height="92"
      ...
      <div class="pa-4 d-flex flex-column align-center ga-3">
        <v-avatar color="white" size="42" class="elevation-2">
          <v-icon icon="mdi-account-tie" color="primary" size="26" />
...
<style scoped>
.acgt-drawer { background: linear-gradient(180deg, #1a237e 0%, #0d1b2a 100%) !important; border: none; }
...
.nav-carre { border-bottom: 1px solid rgba(255,255,255,0.06); border-radius: 0 !important; color: #B0BEC5 !important; }
...
.nav-libelle { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; text-align: center; line-height: 1.2; }
</style>
